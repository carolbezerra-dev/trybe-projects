function puttingStyle(spanHere) {
  ...
  const style = styleClass[from0to3]; // para a linha addClass ficar com menos de 100 (Code Climate);
...
document.querySelector('#criar-carta').addEventListener('click', function () {
  ...
  if (frase === '' || frase === ' ') {
    document.querySelector('#carta-gerada').innerText = 'Please, enter the content of the letter.';
  } else {
  ...
  document.querySelector('#carta-contador').innerText = 
  `Your letter has ${document.querySelectorAll('span').length} caracteres`;
});
